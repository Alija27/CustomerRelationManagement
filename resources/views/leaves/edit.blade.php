...
@section('main')
    ...
    <div  class="w-full bg-white border border-gray-200 shadow-md">
      ...
      <form class="w-11/12 p-6 m-6 overflow-auto bg-white rounded-md " method="POST" enctype="multipart/form-data" action={{route('leaves.update',$leave)}}>
        @csrf
        @method("PUT")
        <div class="mb-6">
            <label class="block font-bold text-gray-600 text-md">Subject</label>
            <input  class="w-full border border-gray-200 " type="text" name="subject" id="subject" value={{$leave->subject}} >
        </div>
        <div class="mb-6">
            <label class="block font-bold text-gray-600 text-md">Image</label>
            <input  class="w-full border border-gray-200" type="file" name="image" id="image" value={{$leave->image}}>
        </div>
        <div class="mb-6">
