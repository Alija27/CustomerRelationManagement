      ...
      <div  class="w-full overflow-auto bg-white border border-gray-200 shadow-md">
        ...
        {{$airline->type}}
         ...
         <td class="text-sm font-medium whitespace-nowrap">
            <a href="{{route('airlines.edit',$airline)}}"> <span class="p-1 px-2 mr-2 text-white bg-blue-800 rounded">   <i class="fa-solid fa-pen-to-square"></i> </span></a>
             <span onclick="show({{$airline->id}})" class="p-1 px-2 mr-2 text-white bg-red-800 rounded cursor-pointer">  <i class="fa-solid fa-trash"></i></a> </span>
             <a href="{{route('airlines.show', $airline->id)}}" ><span  class="p-1 px-2 mr-2 text-white bg-green-800 rounded cursor-pointer">  <i class="fa-solid fa-eye"></i></span></a> 
         </td>
...
<div id="deleteModal" class="fixed top-0 bottom-0 left-0 right-0 hidden bg-black bg-opacity-25 shadow-lg backdrop-blur-lg">
    <div class="flex items-center justify-center">
        <div class="px-10 py-10 font-bold bg-white rounded-lg shadow-lg mt-[15%]">
            ...
            <div class="flex justify-center mt-8 space-x-5">
                <form action="{{route('airlines.delete')}}" method="post">
                    @method("DELETE")
                    @csrf
